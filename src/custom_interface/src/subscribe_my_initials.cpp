 ...
 public:
  ...
  : Node("my_initials_subscriber") {
    ...
    my_initials_subscriber =
      this->create_subscription<custom_interface::msg::MyName>("my_initials", 10,  std::bind(&MyInitialsSubscriber::topic_callback, this, _1));
  }

 private:
  void topic_callback(const custom_interface::msg::MyName & msg) const {
    RCLCPP_INFO(this->get_logger(), "Shelvin heard: '%s'", msg.first_name.c_str());
  }
  rclcpp::Subscription<custom_interface::msg::MyName>::SharedPtr my_initials_subscriber;
};
